...
export default function CourseGraph({
    ...
    const [reactFlowInstance, setReactFlowInstance] =
        React.useState<null | { fitView: (options?: any) => void }>(null);
    ...
    const calculateLayout = useCallback(async () => {
        ...
        const coursesToProcess: [string, PrereqGroup | undefined][] =
            selectedCourse
                ? [[selectedCourse, graphData[selectedCourse]]]
                : visibleCourses.map((course) => [course, graphData[course]]);

        coursesToProcess.forEach(([courseName, prereqs]) => {
            if (!prereqs) return;
    ...
    React.useEffect(() => {
        let cancelled = false;
        (async () => {
            const { nodes: newNodes, edges: newEdges } = await calculateLayout();
            if (cancelled) return;
